...
def create_eyes_image(A_path, size=(256, 256), add_noise=False, pts=None):
    w, h = size
...
def main():
    ...
    parser.add_argument('--in_p', type=str, default='.')
    parser.add_argument('--out', type=str, default='/gpu-data/fpar/eye.png')
    args = parser.parse_args()

    #mkdir(args.out)

    #for lnd in tqdm(sorted(os.listdir(args.in_p))):
        #img = create_eyes_image(os.path.join(args.in_p,lnd))
        #cv2.imwrite(os.path.join(args.out, lnd.replace('.txt', '.png')), img[:,:,::-1])

    img = create_eyes_image(args.in_p)
